...
public class BaseComponentSerializer {
  ...
  protected void deserialize(
      JsonObject object, BaseComponent component, JsonDeserializationContext context) {
    if (object.has("bold")) {
    ...
    if (object.has("hoverEvent")) {
      ...
      HoverEvent.Action action =
          HoverEvent.Action.valueOf(event.get("action").getAsString().toUpperCase(Locale.ROOT));
      ...
      for (String type : Arrays.asList("value", "contents")) {
        ...
        try {
          ...
          if (contents.isJsonArray()) {
            ...
            list =
                new Content[] {context.deserialize(contents, HoverEvent.getClass(action, false))};
          }
  ...
  protected void serialize(
      JsonObject object, BaseComponent component, JsonSerializationContext context) {
    boolean first = false;
    if (es.angelillo15.mast.api.chat.chat.ComponentSerializer.serializedComponents.get() == null) {
      first = true;
      es.angelillo15.mast.api.chat.chat.ComponentSerializer.serializedComponents.set(
          Collections.newSetFromMap(new IdentityHashMap<BaseComponent, Boolean>()));
    }
    try {
      Preconditions.checkArgument(
          !es.angelillo15.mast.api.chat.chat.ComponentSerializer.serializedComponents
              .get()
              .contains(component),
          "Component loop");
      es.angelillo15.mast.api.chat.chat.ComponentSerializer.serializedComponents
      ...
      if (component.getClickEvent() != null) {
        ...
        clickEvent.addProperty(
            "action", component.getClickEvent().getAction().toString().toLowerCase(Locale.ROOT));
      ...
      if (component.getHoverEvent() != null) {
        ...
        hoverEvent.addProperty(
            "action", component.getHoverEvent().getAction().toString().toLowerCase(Locale.ROOT));
        if (component.getHoverEvent().isLegacy()) {
          hoverEvent.add(
              "value", context.serialize(component.getHoverEvent().getContents().get(0)));
        } else {
          hoverEvent.add(
              "contents",
              context.serialize(
                  (component.getHoverEvent().getContents().size() == 1)
                      ? component.getHoverEvent().getContents().get(0)
                      : component.getHoverEvent().getContents()));
        }
