...
public class ConfigLoader {
  ...
  @Getter private static ConfigManager internalStaffItems;
  @Getter private static ConfigManager glow;
  ...
  public void loadConfig() {
    ConfigMerge.merge(
        new File(plugin.getPluginDataFolder().toPath() + File.separator + "config.yml"),
        plugin.getPluginResource("Bukkit/config.yml"));

    config =
        new ConfigManager(plugin.getPluginDataFolder().toPath(), "Bukkit/config.yml", "config.yml");
  ...
  public void loadInternal() {
    internalStaffItems =
        new ConfigManager(
            ...
            "Bukkit/modules/items/internal.yml",
            "/modules/items/internal.yml");
    internalStaffItems.registerConfig();
  }

  public void loadLanguages() {
    File file =
        new File(plugin.getPluginDataFolder().toPath().toString() + File.separator + "lang");
    if (!file.exists()) {
      file.mkdirs();
    }
    ...
    es =
        new ConfigManager(
            plugin.getPluginDataFolder().toPath(), "Bukkit/lang/spanish.yml", "/lang/spanish.yml");
    en =
        new ConfigManager(
            plugin.getPluginDataFolder().toPath(), "Bukkit/lang/english.yml", "/lang/english.yml");
    es.registerConfig();
    en.registerConfig();
  }
  ...
  public void loadPunishmentsGUI() {
    punishmentsGUI =
        new ConfigManager(
            plugin.getPluginDataFolder().toPath(),
            "Bukkit/modules/punishments/gui.yml",
            "/modules/punishments/gui.yml");
  ...
  public void loadGlowModule() {
    glow =
        new ConfigManager(
            plugin.getPluginDataFolder().toPath(), "Bukkit/modules/glow.yml", "/modules/glow.yml");
    glow.registerConfig();
  }
}
