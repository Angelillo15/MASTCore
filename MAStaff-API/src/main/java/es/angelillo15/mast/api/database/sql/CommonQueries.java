...
public class CommonQueries {
  ...
  public static void insertData(UUID uuid, int state) {
    try (PreparedStatement statement =
        connection.prepareStatement(
            "INSERT INTO `staffdata` (`ID`, `UUID`, `state`, `vanished`) VALUES (NULL, ?, ?, ?);")) {
  ...
  public static void updateData(UUID uuid, int state) {
    try (PreparedStatement statement =
        connection.prepareStatement(
            "UPDATE `staffdata` SET `state` = ?, `vanished` = ? WHERE `UUID` = ?;")) {
  ...
  public static boolean existsData(UUID uuid) {
    try (PreparedStatement statement =
        connection.prepareStatement("SELECT * FROM `staffdata` WHERE `UUID` = ?;")) {
      statement.setString(1, uuid.toString());
