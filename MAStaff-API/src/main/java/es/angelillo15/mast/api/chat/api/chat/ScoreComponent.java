 ...
 * It can also be a target selector that <b>must</b> resolve to 1 target, and may target non-player
 * entities. <br>
 * With a book, /tellraw, or /title, using the wildcard '*' in the place of a name or target
 * selector will cause all players to see their own score in the specified objective. <br>
...
public final class ScoreComponent extends BaseComponent {
   ...
   * If not specifically set, value will default to an empty string; signifying that the scoreboard
   * value should take precedence. If not null, nor empty, {@code value} will override any value
   * found in the scoreboard.<br>
