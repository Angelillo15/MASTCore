...
public class CommonSQL extends AbstractDataManager {
  ...
  public void insertUserData(
    ...
    try (PreparedStatement statement =
        PluginConnection.getConnection()
            .prepareStatement(
                "INSERT INTO `mastaff_user_data` (`username`, `UUID`, `last_ip`, `reg_ip`, `first_login`, `last_login`) VALUES (?, ?, ?, ?, ?, ?)")) {
  ...
  public void updateIP(String UUID, String lastIP) {
    try (PreparedStatement statement =
        PluginConnection.getConnection()
            .prepareStatement("UPDATE `mastaff_user_data` SET `last_ip` = ? WHERE `UUID` = ?")) {
  ...
  public void updateLastLogin(String UUID, String lastLogin) {
    try (PreparedStatement statement =
        PluginConnection.getConnection()
            .prepareStatement("UPDATE `mastaff_user_data` SET `last_login` = ? WHERE `UUID` = ?")) {
  ...
  public void updateUsername(String UUID, String username) {
    try (PreparedStatement statement =
        PluginConnection.getConnection()
            .prepareStatement("UPDATE `mastaff_user_data` SET `username` = ? WHERE `UUID` = ?")) {
      statement.setString(1, username);
      statement.setString(2, UUID);
      statement.executeUpdate();
    } catch (Exception e) {
  ...
  public boolean userExists(UUID uuid) {
    try (PreparedStatement statement =
        PluginConnection.getConnection()
            .prepareStatement("SELECT * FROM `mastaff_user_data` WHERE `UUID` = ?")) {
  ...
  public boolean userExists(String username) {
    try (PreparedStatement statement =
        PluginConnection.getConnection()
            .prepareStatement("SELECT * FROM `mastaff_user_data` WHERE `username` = ?")) {
      statement.setString(1, username);
      ResultSet result = statement.executeQuery();
