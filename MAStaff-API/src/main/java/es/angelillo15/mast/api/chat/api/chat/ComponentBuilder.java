 ...
 * </pre>
 *
 * <p>All methods (excluding {@link #append(String)} and {@link #create()} work on the last part
 * appended to the builder, so in the example above "Hello " would be {@link ChatColor#RED} and
 * "World" would be {@link ChatColor#BLUE} but "!" would be bold and {@link ChatColor#BLUE} because
 * append copies the previous part's formatting
 */
@NoArgsConstructor
public final class ComponentBuilder {

  @Getter
  private final List<es.angelillo15.mast.api.chat.api.chat.BaseComponent> parts =
      new ArrayList<es.angelillo15.mast.api.chat.api.chat.BaseComponent>();
  /**
   * The position for the current part to modify. Modified cursors will automatically reset to the
   * last part after appending new components. Default value at -1 to assert that the builder has no
   * parts.
   */
  ...
  private ComponentBuilder(es.angelillo15.mast.api.chat.api.chat.BaseComponent[] parts) {
    for (es.angelillo15.mast.api.chat.api.chat.BaseComponent baseComponent : parts) {
  ...
  public ComponentBuilder(ComponentBuilder original) {
    this(original.parts.toArray(new es.angelillo15.mast.api.chat.api.chat.BaseComponent[0]));
  }
  ...
  public ComponentBuilder(String text) {
    this(new es.angelillo15.mast.api.chat.api.chat.TextComponent(text));
  }
   ...
   */
  public ComponentBuilder(es.angelillo15.mast.api.chat.api.chat.BaseComponent component) {

    this(new es.angelillo15.mast.api.chat.api.chat.BaseComponent[] {component});
  }

  private es.angelillo15.mast.api.chat.api.chat.BaseComponent getDummy() {
    if (dummy == null) {
      dummy =
          new es.angelillo15.mast.api.chat.api.chat.BaseComponent() {
            @Override
            public es.angelillo15.mast.api.chat.api.chat.BaseComponent duplicate() {
              return this;
            }
          };
    }
  ...
  public ComponentBuilder setCursor(int pos) throws IndexOutOfBoundsException {
    if ((this.cursor != pos) && (pos < 0 || pos >= parts.size())) {
      throw new IndexOutOfBoundsException(
          "Cursor out of bounds (expected between 0 + " + (parts.size() - 1) + ")");
    }
  ...
  /**
   * Appends a component to the builder and makes it the current target for formatting. The
   * component will have all the formatting from previous part.
   *
   * @param component the component to append
   * @return this ComponentBuilder for chaining
   */
  public ComponentBuilder append(es.angelillo15.mast.api.chat.api.chat.BaseComponent component) {
    return append(component, FormatRetention.ALL);
  }

  /**
   * Appends a component to the builder and makes it the current target for formatting. You can
  ...
  public ComponentBuilder append(
      es.angelillo15.mast.api.chat.api.chat.BaseComponent component, FormatRetention retention) {
    es.angelillo15.mast.api.chat.api.chat.BaseComponent previous =
        (parts.isEmpty()) ? null : parts.get(parts.size() - 1);
    if (previous == null) {
      previous = dummy;
      dummy = null;
    }
  ...
  /**
   * Appends the components to the builder and makes the last element the current target for
   * formatting. The components will have all the formatting from previous part.
   *
   * @param components the components to append
   * @return this ComponentBuilder for chaining
   */
  public ComponentBuilder append(es.angelillo15.mast.api.chat.api.chat.BaseComponent[] components) {
   ...
   */
  public ComponentBuilder append(
      es.angelillo15.mast.api.chat.api.chat.BaseComponent[] components, FormatRetention retention) {
  ...
  /**
   * Appends the text to the builder and makes it the current target for formatting. The text will
  ...
  /**
   * Parse text to BaseComponent[] with colors and format, appends the text to the builder and makes
   * it the current target for formatting. The component will have all the formatting from previous
   * part.
   *
   * @param text the text to append
   * @return this ComponentBuilder for chaining
  ...
  public ComponentBuilder appendLegacy(String text) {
    return append(es.angelillo15.mast.api.chat.api.chat.TextComponent.fromLegacyText(text));
  }

  /**
   * Appends the text to the builder and makes it the current target for formatting. You can specify
   * the amount of formatting retained from previous part.
   *
   * @param text the text to append
   * @param retention the formatting to retain
   * @return this ComponentBuilder for chaining
  ...
  /**
   * Allows joining additional components to this builder using the given {@link Joiner} and {@link
   * FormatRetention#ALL}.
   *
   * <p>Simply executes the provided joiner on this instance to facilitate a chain pattern.
   *
   ...
   *     size()})
   */
  public es.angelillo15.mast.api.chat.api.chat.BaseComponent getComponent(int pos)
   ...
   */
  public es.angelillo15.mast.api.chat.api.chat.BaseComponent getCurrentComponent() {
   ...
   */
  public es.angelillo15.mast.api.chat.api.chat.BaseComponent[] create() {
    es.angelillo15.mast.api.chat.api.chat.BaseComponent[] cloned =
        new es.angelillo15.mast.api.chat.api.chat.BaseComponent[parts.size()];
    int i = 0;
  ...
  public interface Joiner {
    ...
    /**
     * Joins additional components to the provided {@link ComponentBuilder} and then returns it to
     * fulfill a chain pattern.
     *
     * <p>Retention may be ignored and is to be understood as an optional recommendation to the
     * Joiner and not as a guarantee to have a previous component in builder unmodified.
     *
