...
public class PlayerBanListener implements Listener {
    ...
    if (model.isPermanent()) {
      ...
      return new TextComponent(
          message
              ...
              .replace(
                  "{duration}",
                  TextUtils.formatUptime(model.getUntil() - System.currentTimeMillis()))
              .replace("{banId}", String.valueOf(model.getId())));
    }
  }
  ...
  public void onPostLogin(PostLoginEvent event) {
    ...
    new Thread(
            () -> {
              ...
              if (tableWithUsername != null
                  && tableWithUsername.getUuid() != event.getPlayer().getUniqueId().toString()) {
  ...
  public boolean ipBanCheck(PendingConnection connection) {
    BansTable table =
        IpBansTable.getIpBanned(connection.getAddress().getAddress().getHostAddress());
