...
public class OnServer implements Listener {
    ...
    public void onServerConnectedEvent(ServerConnectedEvent event) {
        MAStaff.getInstance().getPLogger().info(TextUtils.simpleColorize(Messages.getPrefix() + " &rServer connected: &b" + event.getServerName()));
    }

    @EventHandler
    public void onServerDisconnectedEvent(ServerDisconnectedEvent event) {
        MAStaff.getInstance().getPLogger().info(TextUtils.simpleColorize(Messages.getPrefix() + " &rServer disconnected: &b" + event.getServerName()));
    }
}
