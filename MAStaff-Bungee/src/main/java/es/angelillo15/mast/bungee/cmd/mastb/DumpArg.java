        ...
        .forEach(
            (key, addon) -> {
              ...
              addonJson.addProperty(
                  "description",
                  addon.getDescriptionFile().getDescription() == null
                      ? "No description"
                      : addon.getDescriptionFile().getDescription());
    ...
    new Thread(
            () -> {
              try {
                ...
                sender.sendMessage(
                    TextUtils.colorize(Messages.getPrefix() + " &aDump url: &6" + url));
              } catch (Exception e) {
                e.printStackTrace();
                sender.sendMessage(
                    TextUtils.colorize(
                        Messages.getPrefix() + " &cError while dumping plugin info"));
              }
            })
        .start();
  }
